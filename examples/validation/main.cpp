...
int main() {
	...
	unsigned char *v_data = stbi_load((get_executable_dir() + "/common/bunny.png").c_str(),
	...
	for (auto backend : portableRT::available_backends()) {
		...
		for (size_t i = 0; i < hits.size(); i++) {
			validation &=
			    (hits[i].t == std::numeric_limits<float>::infinity() ? 0 : 255) == v_data[i];
		}

		file << backend->name() << "," << backend->device_name() << "," << validation << ","
		     << bvh_duration.count() / 1000.0f << "," << traverse_duration.count() / 1000.0f
		     << "\n";
	}
