...
public:
  ...
  static void divide_sah(const IdxTriVector &input, IdxTriVector &left,
                         IdxTriVector &right) {
    ...
    for (int bin = 0; bin < bin_count; ++bin) {
      for (int axis = 0; axis < 3; ++axis) {
        ...
        float bin_pos = node_aabb.first[axis] +
                        (node_aabb.second[axis] - node_aabb.first[axis]) *
  ...
  static uint32_t build_aux(const IdxTriVector &input,
  ...
  static bool ray_box_intersect(const Ray &ray,
                                const std::array<float, 3> &b_min,
