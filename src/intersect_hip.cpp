...
__global__ void kernel(void* bvh, uint4* out, float4 ray_o, float4 ray_d) {
    ...
    auto res = __builtin_amdgcn_image_bvh_intersect_ray_l(0, ray_extent, ray_o.data, ray_d.data, ray_id.data, desc.data);
    out->x = res[0];
    out->y = res[1];
    out->z = res[2];
    out->w = res[3];
...
bool HIPBackend::intersect_tri(const std::array<float, 9> &v, const Ray &ray) const{
    ...
    uint4* dHit; CHK( hipMalloc(&dHit,sizeof(uint4)) );
